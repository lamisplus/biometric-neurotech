...
public final class LibraryManager {
    ...
    public static void initLibraryPath() {
        ...
        if (Utils.isNullOrEmpty(jnaLibraryPath)) System.setProperty("jna.library.path", libraryPath);
        else {
            System.setProperty("jna.library.path",
                               String.format("%s%s%s", jnaLibraryPath, Utils.PATH_SEPARATOR, libraryPath));
        }
        System.setProperty("java.library.path",
                           String.format("%s%s%s", System.getProperty("java.library.path"), Utils.PATH_SEPARATOR,
                                         libraryPath));
        if (Platform.isMac()) {
            ...
            if (Utils.isNullOrEmpty(jnaPlatformLibraryPath))
                ...
                System.setProperty("jna.platform.library.path",
                                   String.format("%s%s%s", jnaPlatformLibraryPath, Utils.PATH_SEPARATOR, libraryPath));
            }
            System.setProperty("java.platform.library.path",
                               String.format("%s%s%s", System.getProperty("java.platform.library.path"),
                                             Utils.PATH_SEPARATOR, libraryPath));
        }
        try {
    ...
    public static String getLibraryPath() {
        ...
        int index = Utils
                .getWorkingDirectory()
                .lastIndexOf(Utils.FILE_SEPARATOR);
        if (index == -1) {
            return null;
        }
        String part = Utils
